...
const getReviewModalKey = ({ product, initialReview }) => {
  return `${product?.id || 'product'}-${initialReview?.id || 'new-review'}`;
};

function ReviewModalContent({ onClose, product, initialReview, onSubmitReview }) {
  const initialState = createInitialReviewState(initialReview);
  const [rating, setRating] = useState(initialState.rating);
  const [hoverRating, setHoverRating] = useState(initialState.hoverRating);
...
export default function ReviewModal({ isOpen, onClose, product, initialReview, onSubmitReview }) {
  ...
  return (
    <ReviewModalContent
      key={getReviewModalKey({ product, initialReview })}
      onClose={onClose}
      product={product}
