  ...
  return (
    ...
    >
      ...
      >
        ...
        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={`text-slate-400 transition-transform shrink-0 ${isOpen ? 'rotate-180' : ''}`}><path d="m6 9 6 6 6-6"/></svg>
      </div>
      {isOpen && (
...
export { CustomSelect, CITY_OPTIONS, STATE_OPTIONS };

export default function CheckoutForm({
  contactInfo, isEditingContact, isSavingContact, editContactForm, setEditContactForm,
  handleEditContact, handleCancelContact, handleSaveContact,
  addresses, selectedAddress, setSelectedAddress,
  isAddingNewAddress, setIsAddingNewAddress,
  street, setStreet, city, setCity, state, setState,
  addressErrors, setAddressErrors, handleSaveNewAddress,
          ...
          <div className="space-y-4 animate-in fade-in slide-in-from-top-2 duration-200">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <input className="w-full px-3 py-2.5 rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 focus:ring-2 focus:ring-primary/50 focus:border-primary text-sm outline-none transition-all text-slate-700 dark:text-slate-300" type="text" value={editContactForm.name} onChange={(e) => setEditContactForm({...editContactForm, name: e.target.value})} />
              </div>
              <div>
                <label className="block text-xs font-medium text-slate-500 dark:text-slate-400 mb-1">Email Address</label>
                <input className="w-full px-3 py-2.5 rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 focus:ring-2 focus:ring-primary/50 focus:border-primary text-sm outline-none transition-all text-slate-700 dark:text-slate-300" type="email" value={editContactForm.email} onChange={(e) => setEditContactForm({...editContactForm, email: e.target.value})} />
              </div>
              <div className="md:col-span-2">
                <label className="block text-xs font-medium text-slate-500 dark:text-slate-400 mb-1">Phone Number</label>
                <input className="w-full px-3 py-2.5 rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 focus:ring-2 focus:ring-primary/50 focus:border-primary text-sm outline-none transition-all text-slate-700 dark:text-slate-300" type="tel" value={editContactForm.phone} onChange={(e) => setEditContactForm({...editContactForm, phone: e.target.value})} />
              </div>
            </div>
            <div className="flex justify-end gap-3 pt-2">
              <button onClick={handleCancelContact} disabled={isSavingContact} className="px-4 py-2 text-sm font-medium text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg transition-colors disabled:opacity-50">Cancel</button>
              <button onClick={handleSaveContact} disabled={isSavingContact} className="px-4 py-2 text-sm font-medium bg-primary text-white rounded-lg hover:bg-primary/90 transition-colors flex items-center gap-2 disabled:opacity-70">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
          {addresses.map((addr) => (
            <label key={addr.id} className="relative cursor-pointer">
              <input className="peer sr-only" name="address" type="radio" value={addr.id} checked={selectedAddress === addr.id}
                onChange={() => { setSelectedAddress(addr.id); setStreet(''); setCity(''); setState(''); setAddressErrors({}); setIsAddingNewAddress(false); }}
              />
              <div className="h-full p-4 rounded-xl border-2 border-slate-100 dark:border-slate-800 peer-checked:border-primary peer-checked:bg-primary/5 transition-all">
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400 leading-relaxed">{addr.street}<br />{addr.city}, {addr.state} {addr.zip}<br />{addr.country}</p>
              </div>
            </label>
          ))}
        </div>
        {!isAddingNewAddress && addresses.length > 0 ? (
          <button onClick={() => setIsAddingNewAddress(true)} className="w-full py-3 px-4 border-2 border-dashed border-slate-200 dark:border-slate-700 rounded-xl text-sm font-medium text-slate-500 dark:text-slate-400 hover:border-primary hover:text-primary dark:hover:border-primary dark:hover:text-primary transition-colors flex items-center justify-center gap-2">
          ...
          <div className={`animate-in fade-in slide-in-from-top-2 duration-200 ${addresses.length > 0 ? 'pt-6 border-t border-slate-100 dark:border-slate-800' : ''}`}>
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  className={`w-full px-3 py-2.5 rounded-lg border bg-slate-50 dark:bg-slate-800 focus:ring-2 focus:ring-primary/50 focus:border-primary text-sm outline-none transition-all text-slate-700 dark:text-slate-300 placeholder:text-slate-400 ${addressErrors.street ? 'border-red-500 focus:border-red-500 focus:ring-red-500/50' : 'border-slate-200 dark:border-slate-700'}`}
                  placeholder="e.g. 789 Oak Lane" type="text" value={street}
                  onChange={(e) => { setStreet(e.target.value); setSelectedAddress('new'); if (addressErrors.street) setAddressErrors({...addressErrors, street: null}); }}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div onClick={() => setSelectedAddress('new')}>
                  ...
                  <div className={addressErrors.city ? "rounded-lg ring-1 ring-red-500" : ""}>
                    <CustomSelect placeholder="Select City" value={city} onChange={(val) => { setCity(val); setSelectedAddress('new'); if (addressErrors.city) setAddressErrors({...addressErrors, city: null}); }} options={CITY_OPTIONS} />
                  </div>
                ...
                <div onClick={() => setSelectedAddress('new')}>
                  ...
                  <div className={addressErrors.state ? "rounded-lg ring-1 ring-red-500" : ""}>
                    <CustomSelect placeholder="Select State" value={state} onChange={(val) => { setState(val); setSelectedAddress('new'); if (addressErrors.state) setAddressErrors({...addressErrors, state: null}); }} options={STATE_OPTIONS} />
                  </div>
              ...
              <div className="flex justify-end gap-3 pt-2">
                {addresses.length > 0 && (
                  <button onClick={() => { setIsAddingNewAddress(false); setAddressErrors({}); }} className="px-4 py-2 text-sm font-medium text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg transition-colors">Cancel</button>
                )}
                <button onClick={handleSaveNewAddress} className="px-4 py-2 text-sm font-medium bg-primary text-white rounded-lg hover:bg-primary/90 transition-colors">Save Address</button>
              </div>
            </div>
          </div>
        )}
        ...
        <div className="space-y-3">
          <label className="flex items-center justify-between p-4 rounded-xl border border-slate-200 dark:border-slate-700 cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
            <div className="flex items-center gap-3">
              <input className="radio-custom h-5 w-5 text-primary border-slate-300 focus:ring-primary" name="shipping" type="radio" checked={shippingFee === 5.00} onChange={() => setShippingFee(5.00)} />
              <div>
          ...
          <label className="flex items-center justify-between p-4 rounded-xl border border-slate-200 dark:border-slate-700 cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
            <div className="flex items-center gap-3">
              <input className="radio-custom h-5 w-5 text-primary border-slate-300 focus:ring-primary" name="shipping" type="radio" checked={shippingFee === 15.00} onChange={() => setShippingFee(15.00)} />
              <div>
      ...
      <section className="bg-white dark:bg-slate-900 rounded-xl shadow-sm p-6 border border-slate-100 dark:border-slate-800">
        ...
        <div className="space-y-4">
          <label className={`flex items-center gap-3 p-4 rounded-xl border-2 cursor-pointer transition-all ${paymentMethod === 'cod' ? 'border-primary bg-primary/5' : 'border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800'}`}>
            <input className="radio-custom h-5 w-5 text-primary border-slate-300 focus:ring-primary" name="payment" type="radio" value="cod" checked={paymentMethod === 'cod'} onChange={() => setPaymentMethod('cod')} />
          ...
          <div className={`rounded-xl border-2 transition-all ${paymentMethod === 'bank' ? 'border-primary bg-primary/5 p-4' : 'border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800 p-4'}`}>
            <label className={`flex items-center gap-3 cursor-pointer ${paymentMethod === 'bank' ? 'mb-4' : ''}`}>
              <input className="radio-custom h-5 w-5 text-primary border-slate-300 focus:ring-primary" name="payment" type="radio" value="bank" checked={paymentMethod === 'bank'} onChange={() => setPaymentMethod('bank')} />
            ...
            {paymentMethod === 'bank' && (
              <div className="flex flex-col items-center bg-white dark:bg-slate-800 p-6 rounded-lg border border-primary/20 animate-in fade-in slide-in-from-top-2 duration-200">
                ...
                <div className="w-40 h-40 bg-slate-100 dark:bg-slate-700 rounded-lg flex items-center justify-center relative overflow-hidden">
                  <div className="absolute inset-0 bg-linear-to-br from-slate-200 to-slate-400 opacity-20"></div>
